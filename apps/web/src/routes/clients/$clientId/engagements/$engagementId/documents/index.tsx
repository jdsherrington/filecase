...
export const Route = createFileRoute(
  ...
  component: EngagementDocumentsPage,
});
...
function EngagementDocumentsPage() {
  ...
  const bulkMutation = useMutation({
    mutationFn: async (nextStatus: "uploaded" | "in_review" | "final") => {
      return bulkUpdateDocumentStatusServerFn({
        data: {
    ...
    onSuccess: async () => {
      ...
      await queryClient.invalidateQueries({
        queryKey: ["engagement-documents", clientId, engagementId],
      });
    },
  });
  ...
  const columns = [
    ...
    columnHelper.accessor("latestVersion", { header: "Latest Version" }),
    columnHelper.accessor("updatedAt", {
      header: "Updated",
      cell: (info) => new Date(info.getValue()).toLocaleString(),
    }),
    columnHelper.accessor("latestUploadedBy", { header: "Uploaded By" }),
    columnHelper.display({
      id: "details",
      header: "Details",
      cell: ({ row }) => (
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-6 px-6 py-12">
      ...
      <section className="grid gap-3 rounded-lg border p-4 md:grid-cols-4">
        ...
        />
        <Input
          placeholder="File type (mime contains)"
          value={fileType}
          onChange={(event) => {
            setOffset(0);
        ...
        <section className="flex items-center justify-between rounded-lg border p-4">
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              type="button"
              variant="outline"
              onClick={() => bulkMutation.mutate("uploaded")}
            >
              Set uploaded
            </Button>
            <Button
              ...
              type="button"
              variant="outline"
              onClick={() => bulkMutation.mutate("in_review")}
            >
              Set in_review
            </Button>
            <Button
              disabled={bulkMutation.isPending}
              type="button"
              onClick={() => bulkMutation.mutate("final")}
            >
              Set final
            </Button>
          </div>
