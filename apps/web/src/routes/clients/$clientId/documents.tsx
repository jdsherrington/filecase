...
export const Route = createFileRoute("/clients/$clientId/documents")({
  beforeLoad: async () => {
        ...
        <input
          ...
          onChange={() => {
            setSelectedIds((current) => {
              if (current.includes(row.original.id)) {
                return current.filter((id) => id !== row.original.id);
              }

              return [...current, row.original.id];
            });
          }}
        />
      ),
    ...
    columnHelper.display({
      ...
      cell: ({ row }) => (
        <Link
          ...
          params={{ clientId, engagementId: row.original.engagementId }}
